...
def num2words_ru(n):
    ...
    tens = ['', '', 'двадцать', 'тридцать', 'сорок', 'пятьдесят', 'шестьдесят', 'семьдесят', 'восемьдесят', 'девяно0сто']
        ...
        if request.method == 'POST':
            ...
            for order, bid in enumerate(block_ids):
                ...
                if b_type == 'cover':
                    ...
                    s_json = json.dumps({'bg': request.form.get(f'txt_bg_{bid}'), 'color': request.form.get(f'txt_color_{bid}'), 'bc': request.form.get(f'txt_bc_{bid}'), 'bw': request.form.get(f'txt_bw_{bid}')}, ensure_ascii=False)
        ...
        if request.method == 'POST':
            ...
            for order, bid in enumerate(block_ids):
                ...
                if b_type == 'cover':
                    ...
                    items_json = json.dumps({'bg': request.form.get(f'txt_bg_{bid}', '#f9fafb'), 'color': request.form.get(f'txt_color_{bid}', '#333333'), 'bc': request.form.get(f'txt_bc_{bid}', '#2563eb'), 'bw': request.form.get(f'txt_bw_{bid}', 4)})
                elif b_type == 'manual_product':
...
def get_local_icons():
    # Путь к папке с иконками категорий
    folder = os.path.join(app.static_folder, 'category_icons')
    if not os.path.exists(folder):
        os.makedirs(folder)
    # Получаем список всех картинок
    icons = [f for f in os.listdir(folder) if f.endswith(('.png', '.svg', '.jpg', '.webp'))]
